...
from app.projects import (get_project, check_places_limit,
                          fetch_place_from_api, update_project_completion)
...
def add_place(session: SessionDep, project_id: int, place: PlaceCreate) -> Any:
...
def get_places(session: SessionDep, project_id: int) -> Any:
    return session.exec(
...
def get_place(session: SessionDep, project_id: int, place_id: int) -> Any:
...
def update_place(
        ...
        project_id: int,
        place_id: int,
        place: PlaceUpdate) -> Any:

    place_db = get_place(session, project_id, place_id)
